...
//		{
//			byte[] b = (byte[])object;
//			System.out.println("byte line start");
//			for(int i=0;i<b.length;i++)
//			{
//				System.out.print(b[i]);
//				System.out.print(" ");
//			}
//				System.out.println(" end one line ");
//		}else if (object instanceof String)
//		{
//			System.out.println("string line start");
		...
		try
		{
			...
			case ValueMetaInterface.TYPE_STRING : 
				...
				if (v.getLength()<DatabaseMeta.CLOB_LENGTH)
				{
					if (!v.isNull(object)) 
					{
						if(v.getString(object).length()==1)
						{
							ps.setString(pos, v.getString(object));
						}
						else
						{
							String str=v.getString(object).trim();// only trim when length >= 2
				...
				else
				{
					if (!v.isNull(object))
					{
            //String string = v.getString(object); //jason, trim before insert into db
            String string = v.getString(object).trim();
...
//						debug="String, setting ["+logging+"] on position #"+pos+" of the prepared statement";
//	                    System.out.println("===:"+debug);
	...
	public String getInsertStatement(String schemaName, String tableName, RowMetaInterface fields)
	{
        ...
        System.out.println(databaseMeta.getDriverClass());
        if(databaseMeta.getDriverClass().equals("org.apache.phoenix.jdbc.PhoenixDriver"))//jason, phonix jdbc driver
   ...
   if(log.isDetailed()) log.logDetailed(nrstats+" statement"+(nrstats==1?"":"s")+" executed");
	...
	private RowMetaInterface getQueryFieldsFallback(String sql, boolean param, RowMetaInterface inform, Object[] data) throws KettleDatabaseException
	{
		...
		try
		{
			...
			else if(databaseMeta.getPluginId()!=null && databaseMeta.getPluginId().contains("HIVE"))
			{
				String databaseName = databaseMeta.getDatabaseName();
				String tableName  =sql.substring(sql.indexOf("FROM")+4,sql.length()).trim();
				ResultSet rs= null;
				try{
				rs = getDatabaseMetaData().getColumns(null, databaseName, tableName, null);//databaseName is same as schemaName for Hive db
				fields =  getRowInfoFromDatabaseMetadta(rs,true,true);				
				}catch (Exception ex)
				{
					ex.printStackTrace();
				}finally
			...
			else
			{
				...
				try{
					...
					e.printStackTrace();
					log.logError("Current jdbc dose not support PreparedStatement.getMetaData(),use ResultSet.getMetaData() instead");
					rsmt = r.getMetaData();
				}
//				fields=getRowInfo(ps.getMetaData(), false, false);
				fields=getRowInfo(rsmt, false, false);
				//xnren end
				r.close();
				ps.close();
			}
     ...
     * @param lazyConversion true if lazy conversion needs to be enabled where possible
	 */
	private RowMetaInterface getRowInfoFromDatabaseMetadta(ResultSet rs, boolean ignoreLength, boolean lazyConversion) throws KettleDatabaseException
	{
        if (rs==null) {
        	throw new KettleDatabaseException("No result set metadata available to retrieve row metadata!");
        }
...
//				if(columnName.equals("usr_nbr"))
//				{
//					System.out.println("column usr_nbr");
...
//				}
				int type =  new Integer(rs.getString(5)).intValue();//5. DATA_TYPE 
				String originalColumnTypeName= rs.getString(6);//6. TYPE_NAME
				int length= rs.getString(7)==null?0:new Integer(rs.getString(7)).intValue();//7. COLUMN_SIZE 
				int scale= rs.getString(9)==null?0:new Integer(rs.getString(9)).intValue();//9. DECIMAL_DIGITS
...
//		        	}
				...
				ValueMetaInterface v=new ValueMeta(columnName, valtype);
		        v.setLength(length);
		        v.setPrecision(precision);
		        v.setComments("");
		        v.setLargeTextField(isClob);
		        v.setOriginalColumnType(valtype);
		        v.setOriginalColumnTypeName(originalColumnTypeName);
		        v.setOriginalPrecision(precision);
		        v.setOriginalScale(precision);
	...
	private ValueMetaInterface getValueFromSQLType(String name, ResultSetMetaData rm, int index, boolean ignoreLength, boolean lazyConversion) throws SQLException
    {
        ...
        //xnren start 
        //to fix hive which not support isSigned(int)
        boolean signed = false;
        try{
        	signed = rm.isSigned(index);
        }catch(Exception e){
        	log.logError("Set signed as false because of this jdbc dose not support method isSinged(int)");
        }
        ...
        case java.sql.Types.LONGVARBINARY:
            ...
            if ( databaseMeta.isMySQLVariant() &&
                    ...
                    )
                 {
                     ...
                     //valtype = ValueMetaInterface.TYPE_STRING; //jason 20131215  binary values was converted to string and get '?' for some bytes
        ...
        try{
        	...
        	log.logError("Set originalSigned as false because of this jdbc dose not support method isSinged(int)");
        }
	...
	{
        if (rowMeta==null)  //jason 20170110
        {
  ...
  public void printSQLException(SQLException ex)
	...
	public RowMetaAndData getOneRow(String sql, RowMetaInterface param, Object[] data) throws KettleDatabaseException
	{
		...
		{		
        	...
        	try {
        	    //Add by jyhe
        	    if(!(databaseMeta.getDatabaseInterface() instanceof DB2DatabaseMeta && null==row))
        	    {
			...
			if (valueMeta.isNull(valueData)) {
				...
				case ValueMetaInterface.TYPE_DATE:
					...
					if (Const.isEmpty(dateFormat))
						...
						//xnren add db2 dateformat
						else if(databaseMeta.getDatabaseInterface() instanceof DB2DatabaseMeta) {
							if (fieldDateFormatters[i]==null) {
								fieldDateFormatters[i]=new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
							}
							ins.append("'" +  fieldDateFormatters[i].format(date)+ "'") ;
						} 
						//xnren end
						else {
