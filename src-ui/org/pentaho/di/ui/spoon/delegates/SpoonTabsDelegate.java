  ...
  public TabMapEntry findTabForTransformation(TransMeta trans) throws KettleFileException {
    ...
    for (TabMapEntry entry : tabMap) {
      ...
      if (trans.getFilename() != null && entry.getFilename() != null) {
        ...
        //fix bug:when open a trans from a jobEntryTrans, the trans has the same objectId with the job, the trans will not open
        if(meta != null && trans.getObjectId().equals(meta.getObjectId()) && trans.getClass().equals(meta.getClass())) {
		...
		for (TabMapEntry entry : list)
		{
			...
			if (managedObject != null)
			{
				if (entry.getObject() instanceof TransGraph)
				{
					...
					//xnren start 2015-05-27
        			//fix bug:when you create two job or trans with the same name in different folders,than you close one, the step area will be empty and may generate other issues
					entry.getTabItem().setId(tabText);
					entry.setObjectName(tabText);
					for(TabMapEntry entry2 : list){
						if(!entry.equals(entry2) && entry.getTabItem().getText().equals(tabText)){
							entry.setShowingLocation(true);
							tabText = makeTabName(transMeta, entry.isShowingLocation());
							entry.getTabItem().setText(tabText);
							entry.getTabItem().setId(tabText);
							entry.setObjectName(tabText);
						}
				...
				else if (entry.getObject() instanceof JobGraph)
				{
					...
					String tabText = makeTabName(jobMeta, entry.isShowingLocation());
					entry.getTabItem().setText(tabText);
					//xnren start 2015-05-27
        			//fix bug:when you create two job or trans with the same name in different folders,than you close one, the step area will be empty and may generate other issues
					entry.getTabItem().setId(tabText);
					...
					for(TabMapEntry entry2 : list){
						if(!entry.equals(entry2) && entry.getTabItem().getText().equals(tabText)){
							entry.setShowingLocation(true);
							tabText = makeTabName(jobMeta, entry.isShowingLocation());
							entry.getTabItem().setText(tabText);
							entry.getTabItem().setId(tabText);
							entry.setObjectName(tabText);
						}
