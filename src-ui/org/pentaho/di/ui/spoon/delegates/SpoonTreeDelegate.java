			...
			for (int s = 0; s < selection.length; s++)
			{
				...
				case 3: // ------complete-----
					if (path[0].equals(Spoon.STRING_TRANSFORMATIONS)) // Transformations title
					{
						...
						if (path[2].equals(Spoon.STRING_CACHE_FILE))
							object = new TreeSelection(path[2], CacheFile.class, transMeta);
					}
					if (path[0].equals(Spoon.STRING_JOBS)) // Jobs title
					{
				...
				case 4: // ------complete-----
					if (path[0].equals(Spoon.STRING_TRANSFORMATIONS)) // The name of a transformation
					{
						...
						if (transMeta!=null) {
							...
							if (path[2].equals(Spoon.STRING_CACHE_FILE))//add by cli   add pop up to single cache file
								object = new TreeSelection(path[3], transMeta.findCacheFile(path[3]), transMeta);
						}
					}
