...
public class TransGraph extends AbstractGraph implements XulEventHandler, Redrawable, TabItemInterface, LogParentProvidedInterface, MouseListener, MouseMoveListener, MouseTrackListener, MouseWheelListener, KeyListener {
  ...
  private org.pentaho.ui.xul.dom.Document doc;
    ...
    private void addCandidateAsHop(int mouseX, int mouseY) {
    	...
    	if (forward) {
    		if (fromIoMeta.isOutputProducer() && toStep.equals(currentStep)) {
    			streams.add( new Stream(StreamType.OUTPUT, fromStep, BaseMessages.getString(PKG,"Spoon.MainOutputOfStep"), StreamIcon.OUTPUT, null) ); //$NON-NLS-1$
    		}

        	if (fromStep.supportsErrorHandling() && toStep.equals(currentStep)) {
        		streams.add( new Stream(StreamType.ERROR, fromStep, BaseMessages.getString(PKG,"Spoon.ErrorHandlingOfStep"), StreamIcon.ERROR, null) ); //$NON-NLS-1$
        	}
    	} else {
	    	if (toIoMeta.isInputAcceptor() && fromStep.equals(currentStep)) {
	    		streams.add( new Stream(StreamType.INPUT, toStep, BaseMessages.getString(PKG,"Spoon.MainInputOfStep"), StreamIcon.INPUT, null) ); //$NON-NLS-1$
	    	}
...
//    }
   ...
   * add menu control
   */
  private void allTransMenusControl(){
		/**
		 * 转换工具栏按钮及右键菜单
		 */
		//转换工具栏按钮
		if(!checkPrivilege(MenuPrivileges.BUTTON_TRANS_RUN))disableMenuItem(doc, "trans-run", true) ;
		if(!checkPrivilege(MenuPrivileges.BUTTON_TRANS_PAUSE))disableMenuItem(doc, "trans-pause", true) ;
		if(!checkPrivilege(MenuPrivileges.BUTTON_TRANS_STOP))disableMenuItem(doc, "trans-stop", true) ;
		if(!checkPrivilege(MenuPrivileges.BUTTON_TRANS_PREVIEW))disableMenuItem(doc, "trans-preview", true) ;
		if(!checkPrivilege(MenuPrivileges.BUTTON_TRANS_DEBUG))disableMenuItem(doc, "trans-debug", true) ;
		if(!checkPrivilege(MenuPrivileges.BUTTON_TRANS_REPLAY))disableMenuItem(doc, "trans-replay", true) ;
		if(!checkPrivilege(MenuPrivileges.BUTTON_TRANS_VERIFY))disableMenuItem(doc, "trans-verify", true) ;
		if(!checkPrivilege(MenuPrivileges.BUTTON_TRANS_IMPACT))disableMenuItem(doc, "trans-impact", true) ;
		if(!checkPrivilege(MenuPrivileges.BUTTON_TRANS_GET_SQL))disableMenuItem(doc, "trans-get-sql", true) ;
		if(!checkPrivilege(MenuPrivileges.BUTTON_TRANS_EXPLORE_DATABASE))disableMenuItem(doc, "explore-database", true) ;
		if(!checkPrivilege(MenuPrivileges.BUTTON_TRANS_SHOW_RESULTS))disableMenuItem(doc, "trans-show-results", true) ;
		//转换步骤右键菜单
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_ENTRY_NEWHOP))disableMenuItem(doc, "trans-graph-entry-newhop", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_ENTRY_OPEN_MAPPING))disableMenuItem(doc, "trans-graph-entry-open-mapping", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_ENTRY_EDIT))disableMenuItem(doc, "trans-graph-entry-edit", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_ENTRY_EDIT_DESCRIPTION))disableMenuItem(doc, "trans-graph-entry-edit-description", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_ENTRY_DATA_MOVEMENT))disableMenuItem(doc, "trans-graph-entry-data-movement", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_ENTRY_COPIES))disableMenuItem(doc, "trans-graph-entry-copies", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_ENTRY_COPY))disableMenuItem(doc, "trans-graph-entry-copy", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_ENTRY_DUPLICATE))disableMenuItem(doc, "trans-graph-entry-duplicate", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_ENTRY_DELETE))disableMenuItem(doc, "trans-graph-entry-delete", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_ENTRY_HIDE))disableMenuItem(doc, "trans-graph-entry-hide", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_ENTRY_DETACH))disableMenuItem(doc, "trans-graph-entry-detach", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_ENTRY_INPUTS))disableMenuItem(doc, "trans-graph-entry-inputs", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_ENTRY_OUTPUTS))disableMenuItem(doc, "trans-graph-entry-outputs", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_ENTRY_SNIFF))disableMenuItem(doc, "trans-graph-entry-sniff", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_ENTRY_ALIGN))disableMenuItem(doc, "trans-graph-entry-align", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_ENTRY_VERIFY))disableMenuItem(doc, "trans-graph-entry-verify", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_ENTRY_MAPPING))disableMenuItem(doc, "trans-graph-entry-mapping", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_ENTRY_PARTITIONING))disableMenuItem(doc, "trans-graph-entry-partitioning", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_ENTRY_CLUSTERING))disableMenuItem(doc, "trans-graph-entry-clustering", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_ENTRY_ERRORS))disableMenuItem(doc, "trans-graph-entry-errors", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_ENTRY_PREVIEW))disableMenuItem(doc, "trans-graph-entry-preview", true) ;
		//连接右键菜单
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_HOP_EDIT))disableMenuItem(doc, "trans-graph-hop-edit", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_HOP_FLIP))disableMenuItem(doc, "trans-graph-hop-flip", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_HOP_ENABLED))disableMenuItem(doc, "trans-graph-hop-enabled", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_HOP_DELETE))disableMenuItem(doc, "trans-graph-hop-delete", true) ;
		//disableMenuItem(doc, "trans-graph-hop-bulk", !checkPrivilege(TRANS_GRAPH_HOP_BULK)) ;
		//注释右键菜单
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_NOTE_EDIT))disableMenuItem(doc, "trans-graph-note-edit", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_NOTE_DELETE))disableMenuItem(doc, "trans-graph-note-delete", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_NOTE_RAISE))disableMenuItem(doc, "trans-graph-note-raise", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_NOTE_LOWER))disableMenuItem(doc, "trans-graph-note-lower", true) ;
		//转换设计区右键菜单
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_BACKGROUND_NEW_NOTE))disableMenuItem(doc, "trans-graph-background-new-note", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_BACKGROUND_PAUSE))disableMenuItem(doc, "trans-graph-background-paste", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_BACKGROUND_SELECT_ALL))disableMenuItem(doc, "trans-graph-background-select-all", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_BACKGROUND_CLEAR_SELECTION))disableMenuItem(doc, "trans-graph-background-clear-selection", true) ;
		if(!checkPrivilege(MenuPrivileges.TRANS_GRAPH_BACKGROUND_SETTIONGS))disableMenuItem(doc, "trans-graph-background-settings", true) ;
  }
    ...
    if (((Control)toolbar.getManagedObject()).isDisposed()) return;
      ...
      getDisplay().asyncExec(new Runnable() {
        ...
        public void run() {
          ...
          //edit by cli
      	  //add trans menu control
          if(spoon.getRepository()!=null){	
      		allTransMenusControl();
      	  }
          //end edit by cli 
        }
  ...
  private synchronized void disableMenuItem(org.pentaho.ui.xul.dom.Document doc, String itemId, boolean disable) {
