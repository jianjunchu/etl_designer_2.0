...
/**
  ...
  private void allJobMenusControl(){
		/**
		 * 作业工具栏按钮及右键菜单
		 */
		//作业工具栏按钮
	  	//xnren start 2015-05-26
	    //fix: if you do have the privilege, keep the menuitem or button disabled as the setting before, rather than forced disabled=false
		if(!checkPrivilege(MenuPrivileges.JOB_RUN))disableMenuItem(doc, "job-run", true) ;
		if(!checkPrivilege(MenuPrivileges.JOB_STOP))disableMenuItem(doc, "job-stop", true) ;
		if(!checkPrivilege(MenuPrivileges.JOB_REPLAY))disableMenuItem(doc, "job-replay", true) ;
		if(!checkPrivilege(MenuPrivileges.JOB_GET_SQL))disableMenuItem(doc, "job-get-sql", true) ;
		if(!checkPrivilege(MenuPrivileges.JOB_EXPLORE_DATABASE))disableMenuItem(doc, "explore-database", true) ;
		if(!checkPrivilege(MenuPrivileges.JOB_SHOW_RESULT))disableMenuItem(doc, "job-show-results",  true);
		//作业步骤右键菜单
		if(!!checkPrivilege(MenuPrivileges.JOB_GRAPH_ENTRY_NEWHOP))disableMenuItem(doc, "job-graph-entry-newhop", true) ;
		if(!checkPrivilege(MenuPrivileges.JOB_GRAPH_ENTRY_LAUNTH))disableMenuItem(doc, "job-graph-entry-launch", true) ;
		if(!checkPrivilege(MenuPrivileges.JOB_GRAPH_ENTRY_EDIT))disableMenuItem(doc, "job-graph-entry-edit", true) ;
		if(!checkPrivilege(MenuPrivileges.JOB_GRAPH_ENTRY_EDIT_DESCRIPTION))disableMenuItem(doc, "job-graph-entry-edit-description", true) ;
		if(!checkPrivilege(MenuPrivileges.JOB_GRAPH_ENTRY_PARALLEL))disableMenuItem(doc, "job-graph-entry-parallel", true) ;
		if(!checkPrivilege(MenuPrivileges.JOB_GRAPH_ENTRY_DUPLICATE))disableMenuItem(doc, "job-graph-entry-duplicate", true) ;
		if(!checkPrivilege(MenuPrivileges.JOB_GRAPH_ENTRY_COPY))disableMenuItem(doc, "job-graph-entry-copy", true) ;
		if(!checkPrivilege(MenuPrivileges.JOB_GRAPH_ENTRY_ALIGN))disableMenuItem(doc, "job-graph-entry-align", true) ;
		if(!checkPrivilege(MenuPrivileges.JOB_GRAPH_ENTRY_DETACH))disableMenuItem(doc, "job-graph-entry-detach", true) ;
		if(!checkPrivilege(MenuPrivileges.JOB_GRAPH_ENTRY_HIDE))disableMenuItem(doc, "job-graph-entry-hide", true) ;
		if(!checkPrivilege(MenuPrivileges.JOB_GRAPH_ENTRY_DELETE))disableMenuItem(doc, "job-graph-entry-delete", true) ;
		//连接右键菜单
		if(!checkPrivilege(MenuPrivileges.JOB_GRAPH_HOP_EVALUATION))disableMenuItem(doc, "job-graph-hop-evaluation", true) ;
		if(!checkPrivilege(MenuPrivileges.JOB_GRAPH_HOP_FLIP))disableMenuItem(doc, "job-graph-hop-flip", true) ;
		if(!checkPrivilege(MenuPrivileges.JOB_GRAPH_HOP_ENABLED))disableMenuItem(doc, "job-graph-hop-enabled", true) ;
		if(!checkPrivilege(MenuPrivileges.JOB_GRAPH_HOP_DELETE))disableMenuItem(doc, "job-graph-hop-delete", true) ;
		if(!checkPrivilege(MenuPrivileges.JOB_GRAPH_HOP_BULK))disableMenuItem(doc, "job-graph-graph-hop-bulk", true) ;
		//注释右键菜单
		if(!checkPrivilege(MenuPrivileges.JOB_GRAPH_NOTE_EDIT))disableMenuItem(doc, "job-graph-note-edit", true) ;
		if(!checkPrivilege(MenuPrivileges.JOB_GRAPH_NOTE_DELETE))disableMenuItem(doc, "job-graph-note-delete", true) ;
		//作业设计区右键菜单
		if(!checkPrivilege(MenuPrivileges.JOB_GRAPH_BACKGROUND_NOTE_NEW))disableMenuItem(doc, "job-graph-note-new", true) ;
		if(!checkPrivilege(MenuPrivileges.JOB_GRAPH_BACKGROUND_NOTE_PAUSE))disableMenuItem(doc, "job-graph-note-paste", true) ;
		if(!checkPrivilege(MenuPrivileges.JOB_GRAPH_BACKGROUND_SELECT_ALL))disableMenuItem(doc, "job-graph-background-select-all", true) ;
		if(!checkPrivilege(MenuPrivileges.JOB_GRAPH_BACKGROUND_CLEAR_SELECTION))disableMenuItem(doc, "job-graph-background-clear-selection", true) ;
		if(!checkPrivilege(MenuPrivileges.JOB_GRAPH_BACKGROUND_SETTIONGS))disableMenuItem(doc, "job-graph-background-settings", true) ;
		//xnren end
  }
  public void setControlStates() {
      ...
      public void run() {
        ...
        // Pause button...
        // Add by jyhe
        XulToolbarbutton pauseButton = (XulToolbarbutton) toolbar.getElementById("job-pause");
  ...
  public boolean canHandleSave() {
	...
	public HasLogChannelInterface getLogChannelProvider() {
	  ...
	  {
		  if(null!=job && !job.isStopped())
		  {
			  if(!job.getPauseStatus())
				  job.pauseRunning();
			  else
				  job.resumeRunning();
		  }
