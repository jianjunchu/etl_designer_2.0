...
import org.pentaho.di.core.util.ProcessUtil;
...
import org.pentaho.di.www.AddRepServlet;
...
public class Trans implements VariableSpace, NamedParams, HasLogChannelInterface, LoggingObjectInterface
{
      ...
      // Now prepare to start all the threads...
		...
		TransListener transListener = new TransAdapter() {
				public void transFinished(Trans trans) {
					...
					//jason:  then execute after trans shell script
					String afterSubr = transMeta.getCreatedUser();
					ProcessUtil procUtil = new ProcessUtil();
					try {
						procUtil.runProcess(afterSubr,null, null, null, log);
        ...
        try
        {
            ...
            if (masterSteps.size()>0) // If there is something that needs to be done on the master...
            {
                ...
                if (executionConfiguration.isClusterPosting())
                {
                    ...
                    //xnren start set rep parameters
                	if(executionConfiguration.getRepository() instanceof KettleDatabaseRepository){
                		KettleDatabaseRepository rep = (KettleDatabaseRepository)executionConfiguration.getRepository();
                		masterServer.execService(AddRepServlet.CONTEXT_PATH + "/?repName=" + URLEncoder.encode(rep.getName(), "UTF-8") + "&repVersion=" + KettleEngine.VERSION_4_3  + "&repDbType=" + rep.getDatabaseMeta().getDatabaseTypeDesc() + "&repDbHost=" + rep.getDatabaseMeta().getHostname() + "&repDbPort=" + rep.getDatabaseMeta().getDatabasePortNumberString() + "&repDbName=" + rep.getDatabaseMeta().getDatabaseName() + "&repDbUsername=" + rep.getDatabaseMeta().getUsername() + "&repDbPassword=" + rep.getDatabaseMeta().getPassword()); 
                	}
                	//xnren end
                        ...
                        public void run() {
                          try {
                             ...
                             //xnren start set rep parameters
                          	 if(executionConfiguration.getRepository() instanceof KettleDatabaseRepository){
                          		 KettleDatabaseRepository rep = (KettleDatabaseRepository)executionConfiguration.getRepository();
                          		 slaves[index].execService(AddRepServlet.CONTEXT_PATH + "/?repName=" + URLEncoder.encode(rep.getName(), "UTF-8") + "&repVersion=" + KettleEngine.VERSION_4_3  + "&repDbType=" + rep.getDatabaseMeta().getDatabaseTypeDesc() + "&repDbHost=" + rep.getDatabaseMeta().getHostname() + "&repDbPort=" + rep.getDatabaseMeta().getDatabasePortNumberString() + "&repDbName=" + rep.getDatabaseMeta().getDatabaseName() + "&repDbUsername=" + rep.getDatabaseMeta().getUsername() + "&repDbPassword=" + rep.getDatabaseMeta().getPassword()); 
                          	 }
                          	 //xnren end
  ...
  public static final long monitorClusteredTransformation(LogChannelInterface log, TransSplitter transSplitter, Job parentJob, int sleepTimeSeconds) {
	...
	if(repository instanceof KettleDatabaseRepository){
		KettleDatabaseRepository rep = (KettleDatabaseRepository)repository;
		slaveServer.execService(AddRepServlet.CONTEXT_PATH + "/?repName=" + URLEncoder.encode(rep.getName(), "UTF-8") + "&repVersion=" + KettleEngine.VERSION_4_3  + "&repDbType=" + rep.getDatabaseMeta().getDatabaseTypeDesc() + "&repDbHost=" + rep.getDatabaseMeta().getHostname() + "&repDbPort=" + rep.getDatabaseMeta().getDatabasePortNumberString() + "&repDbName=" + rep.getDatabaseMeta().getDatabaseName() + "&repDbUsername=" + rep.getDatabaseMeta().getUsername() + "&repDbPassword=" + rep.getDatabaseMeta().getPassword()); 
	}
	//xnren end
