...
public class ChangeFileEncoding extends BaseStep implements StepInterface
{
    ...
    public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException
    {
        ...
        try
        {
        	...
        	//edit by cli 
        	//change data.sourceFile.getParent().exists() to tartgetFileObject.getParent().exists()
        	FileObject tartgetFileObject = KettleVFS.getFileObject(targetFilename);        	
        	if(!tartgetFileObject.getParent().exists()) {
            	if(meta.isCreateParentFolder()) {
            		tartgetFileObject.getParent().createFolder();
            	}else {
