   ...
   private static Class<?> PKG = SymmetricCrypto.class; // for i18n purposes, needed by Translator2!!   $NON-NLS-1$
	...
	public SymmetricCrypto(SymmetricCryptoMeta meta) throws CryptoException {
		...
		this.scheme= meta.getDefaultScheme();
		 try {
			  if(this.scheme==null) throw new CryptoException(BaseMessages.getString(PKG, "SymmetricCrypto.SchemeMissing"));
	...
	public void setEncryptMode() throws CryptoException 
	{
	  ...
	  public void setSecretKeyFromFile(String filename) throws CryptoKeyException  {		  
		  ...
		  try  {	
			  ...
			  if(!file.exists()) throw new CryptoException(BaseMessages.getString(PKG, "SymmetricCrypto.CanNotFindFile",file.getName()));
