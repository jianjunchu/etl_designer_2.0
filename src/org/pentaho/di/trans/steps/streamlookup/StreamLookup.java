...
public class StreamLookup extends BaseStep implements StepInterface
{
	...
	private boolean readLookupValues() throws KettleException
	{
		...
		while (rowData!=null)
		{
            ...
            if (firstRun)
            {
    			...
    			if(meta.isFromStep() && meta.isUpdateCacheFile())//jjchu add cache 20120802
    	        {
    	        	data.cache = CacheFactory.create(data.infoMeta,  getTransMeta().getCacheFilePath(meta.getUpdateCacheFileName()).getFilePath(),getTransMeta().getCacheFilePath(meta.getUpdateCacheFileName()).getMemorySize());
    	        }
            }
            

            if(meta.isFromStep() && !meta.isUpdateCacheFile()) //jjchu add cache 201208
            {
            ...
            if(meta.isFromStep() && meta.isUpdateCacheFile())//jjchu add cache 20120802
	        {
                ...
                if(data.cache==null || data.cache.hasClosed())
                {
                    if(meta.isFromCacheFile())
                        data.cache = CacheFactory.load(this.getTransMeta().getCacheFilePath(meta.getSearchCacheFileName()).getFilePath());
                    else
                        data.cache = CacheFactory.load(this.getTransMeta().getCacheFilePath(meta.getUpdateCacheFileName()).getFilePath());
                        
                }                
	        	data.cache.insert(rowData);
	        }

			rowData=getRowFrom(rowSet);
		}
		
        if(meta.isFromStep() && meta.isUpdateCacheFile() && data.cache!=null)//jjchu add cache 20120802
        {
        	data.cache.createIndex(data.keyMeta, 0);
        	try 
        	{
        	    /* flush*/
				data.cache.flush() ;
	...
	private Object[] getFromCache(RowMetaInterface keyMeta, Object[] keyData) throws Exception
    {
		if(meta.isFromStep() && !meta.isUpdateCacheFile())//added by jjchu
		{
	        if (meta.isMemoryPreservationActive())
	        {
	            if (meta.isUsingSortedList())
		...
		else
		{
			...
			ResultSet rs= data.cache.search(keyData, data.valueMeta);
			if(rs.next())
			{
			    Object[] values = rs.getAll() ;
			    rs.close() ;
			    return values;
			}
			else
...
//    }
    ...
    public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException
	{
	    ...
	    if (data.readLookupValues)//jjchu 20060208
        ...
        Object[] outputRow =null;
     	outputRow = lookupValues(getInputRowMeta(), r); // Do the actual lookup in the hastable.
