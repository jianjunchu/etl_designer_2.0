...
public class FIFOCalculatorMeta extends BaseStepMeta implements StepMetaInterface
{
	private static Class<?> PKG = FIFOCalculatorMeta.class; // for i18n purposes, needed by Translator2!!   $NON-NLS-1$

    private String dateField;
    private String multiplyField;
	...
	public Object clone()
	{
		FIFOCalculatorMeta retval = (FIFOCalculatorMeta)super.clone();
	...
	public String getXML()
	{
        ...
        retval.append(XMLHandler.addTagValue("flag_field", dateField));         //$NON-NLS-1$
        retval.append(XMLHandler.addTagValue("multiply_field", multiplyField));         //$NON-NLS-1$
		...
		throws KettleXMLException
	{
		try
		{
            ...
            dateField = XMLHandler.getTagValue(stepnode, "flag_field"); //$NON-NLS-1$
            multiplyField = XMLHandler.getTagValue(stepnode, "multiply_field"); //$NON-NLS-1$
            List<StreamInterface> infoStreams = getStepIOMeta().getInfoStreams();
	...
	public void readRep(Repository rep, ObjectId id_step, List<DatabaseMeta> databases, Map<String, Counter> counters) throws KettleException
	{
		try
		{
            ...
            dateField  =   rep.getStepAttributeString (id_step, "flag_field");  //$NON-NLS-1$
            multiplyField =   rep.getStepAttributeString (id_step, "multiply_field");
            List<StreamInterface> infoStreams = getStepIOMeta().getInfoStreams();
            StreamInterface referenceStream = infoStreams.get(0);
//            StreamInterface compareStream = infoStreams.get(1);
...
//			compareStream.setSubject( rep.getStepAttributeString (id_step, "compare") );  //$NON-NLS-1$
	...
	public void saveRep(Repository rep, ObjectId id_transformation, ObjectId id_step) throws KettleException
	{
		try
		{
            ...
            rep.saveStepAttribute(id_transformation, id_step, "flag_field", dateField); //$NON-NLS-1$
            rep.saveStepAttribute(id_transformation, id_step, "multiply_field", multiplyField); //$NON-NLS-1$
            List<StreamInterface> infoStreams = getStepIOMeta().getInfoStreams();
            StreamInterface referenceStream = infoStreams.get(0);
//            StreamInterface compareStream = infoStreams.get(1);
...
//        if (Const.isEmpty(dateField)) throw new KettleStepException(BaseMessages.getString(PKG, "MergeRowsMeta.Exception.FlagFieldNotSpecified"));
//        ValueMetaInterface flagFieldValue = new ValueMeta(dateField, ValueMetaInterface.TYPE_STRING);
...
//        StreamInterface compareStream = infoStreams.get(1);
		...
		if (referenceStream.getStepname()!=null)
		{
	...
	{
		return new FIFOCalculator(stepMeta, stepDataInterface, cnr, tr, trans);
	}

	public StepDataInterface getStepData()
	{
		return new FIFOCalculatorData();
	}
	...
	public String getMultiplyField() {
		return multiplyField;
	}

	public void setMultiplyField(String multiplyField) {
		this.multiplyField = multiplyField;
	}
