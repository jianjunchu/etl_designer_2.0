...
{	
  ...
  final private class StreamLogger extends Thread
  {
		...
		try
		{
			...
			if ( ! preview )
			{
				...
				{					
			        ...
			        if(meta.createTable())
			        {
			        	String tableName =meta.getDatabaseMeta().getQuotedSchemaTableCombination(environmentSubstitute(meta.getSchemaName()),
                                environmentSubstitute(meta.getTableName()));
			        	Database db = new Database(meta.getDatabaseMeta());
			        	db.connect();
			        	if(!db.checkTableExists(tableName))
			        	{
			        		String sqlScript = db.getDDL(tableName,getInputRowMeta(),null, false, null, true);  
    ...
    public boolean init(StepMetaInterface smi, StepDataInterface sdi)
	{
		...
		{			
//JASON 2017
			DatabaseMeta ci = meta.getDatabaseMeta();
			try{
			if (ci==null) return false;
				data.db = new Database(this,ci);	
	            ...
	            logBasic("Connected to Oracle in "+this.getStepname());

		          String sql = environmentSubstitute(meta.getBeforeSQL());
//		          if (meta.isSingleStatement()) {
//						data.result = data.db.execStatement(data.sql);
//					} else {
						Result result = data.db.execStatements(sql);
//					}
...
//			}
			if (!data.db.isAutoCommit()) 
	          ...
	          logError("An error occurred disposeing this step: " + e.getMessage()+" while execute after-SQL:"+sql);
	          stopAll();
	          setErrors(1);
	    }
		//JASON END
		...
		private void execsqlscript(String sqlScript)
		{
	        ...
	        for (int partitionNr=0;first || (partitioningInformation!=null && partitionNr<partitioningInformation.length) ; partitionNr++)
	        {
	          ...
	          message.append(Const.CR);
	    		...
	    		{
	                String error = BaseMessages.getString(PKG, "SQLEditor.Error.CouldNotConnect.Message", (ci==null ? "" : ci.getName()), dbe.getMessage());
	                message.append(error).append(Const.CR);
	    		}
	    		finally
	    		{
