   ...
   private boolean readNextString() {
	   ...
	   try {
		   ...
		   if (data.readrow==null)  {
			// finished processing!
           	if (log.isDetailed()) logDetailed(BaseMessages.getString(PKG, "YamlInput.Log.FinishedProcessing"));
               return false;
           }
		   ...
		   if(first) {
				...
				if (data.indexOfYamlField<0) {
					...
					throw new KettleException(BaseMessages.getString(PKG, "YamlInput.Exception.CouldnotFindField",meta.getYamlField())); //$NON-NLS-1$ //$NON-NLS-2$
				}
			}
		   ...
		   if(log.isDetailed()) logDetailed(BaseMessages.getString(PKG, "YamlInput.Log.YAMLStream", meta.getYamlField(),Fieldvalue));
   ...
   private void addFileToResultFilesname(FileObject file) throws Exception {
	...
	private boolean openNextFile() {
		try {
			...
			if(meta.isIgnoreEmptyFile() && data.file.getContent().getSize()==0){
				if(isBasic()) logBasic(BaseMessages.getString(PKG, "YamlInput.Error.FileSizeZero", data.file.getName()));
				// Let's open the next file
				openNextFile();
				
			}else {
				if (isDetailed()) logDetailed(BaseMessages.getString(PKG, "YamlInput.Log.OpeningFile", data.file.toString()));
	...
	public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException
	{
		...
		if (r==null)  {
		 ...
		 if (log.isRowLevel()) logRowlevel(BaseMessages.getString(PKG, "YamlInput.Log.ReadRow", data.outputRowMeta.getString(r)));
	...
	public void dispose(StepMetaInterface smi, StepDataInterface sdi) {
		...
		data = (YamlInputData) sdi;
		if(data.yaml!=null) {
			try {data.yaml.close();}catch(Exception e){};
		}
		if(data.file!=null) {
			try{
				data.file.close();
