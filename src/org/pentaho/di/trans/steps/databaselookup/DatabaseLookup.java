...
public class DatabaseLookup extends BaseStep implements StepInterface
{
	...
	/**Jason
	 * Performs the lookup based on the meta-data and the input row.
	 * if there are multiple rows returned, get one row by use of fuzzy match.
	 * @param row The row to use as lookup data and the row to add the returned lookup fields to
	 * @return the resulting row after the lookup values where added
	 * @throws KettleException In case something goes wrong.
	 */
	private synchronized Object[] lookupFuzzyMatchValues(RowMetaInterface inputRowMeta, Object[] row) throws KettleException
	{
		Object[] outputRow = RowDataUtil.resizeArray(row, data.outputRowMeta.size());
                
        Object[] lookupRow = new Object[data.lookupMeta.size()];
        Object[] fuzzyMatchRow = new Object[meta.getStreamFuzzyMatchKeyField().length];
        int lookupIndex=0;
        ...
        for (int i=0;i<meta.getStreamFuzzyMatchKeyField().length;i++)
		{
			if (data.keynrs[i]>=0)
			{
				fuzzyMatchRow[i] = row[data.fuzzyMatchKeynrs[i]];
			}
		}
        Object[] add = null;
        boolean cache_now=false;
		...
		if (add==null)
		{
			if ( !(meta.isCached() && meta.isLoadingAllDataInCache()) || data.hasDBCondition ) { // do not go to the database when all rows are in (exception LIKE operator)
				...
				ResultSet rs = null;
				try {
					rs = data.db.getPrepStatementLookup().executeQuery();
					while (rs.next())
					{
						String tableValue = rs.getString(meta.getTableFuzzyMatchKeyField()[0]);
						double csimilarity = new Double(0);
						int type = FuzzyMatchMeta.getAlgorithmTypeByDesc(meta.getMatchMethods()[0].toString());
						switch (type) {
			    		case FuzzyMatchMeta.OPERATION_TYPE_JARO:
			    			csimilarity= new Jaro().score(tableValue,fuzzyMatchRow[0].toString());
			    		break;
			    		case FuzzyMatchMeta.OPERATION_TYPE_JARO_WINKLER:
			    			csimilarity= new JaroWinkler().score(tableValue,lookupRow[0].toString());
			    		break;
			    		default:
			    			// Letters pair similarity	
			    			csimilarity=LetterPairSimilarity.getSimiliarity(tableValue,lookupRow[0].toString());	
			    		break;
					}
			    		if(csimilarity < new Double(meta.getMinValues()[0].toString()).doubleValue())
			    		{
			    			continue;
			    		}else
			    		{
			    			add = new Object[meta.getReturnValueField().length] ;
			    			for(int i=0;i<meta.getReturnValueField().length;i++)
			    			{
			    				if(meta.getReturnValueNewName()[i]!=null && meta.getReturnValueNewName()[i].length()>0 )
			    					add[i]=rs.getString(meta.getReturnValueNewName()[i]);
			    				else
			    					add[i]=rs.getString(meta.getReturnValueField()[i]);
			    			}
			    			break;
			    		}
					}
        ...
        else
        {
        	...
        	if (!cacheHit)
        	{
	            ...
	            for (int i=0;i<types.length;i++)
	        	{  
	            	//ValueMetaInterface = data.db.getReturnRowMeta().getValueMeta(i);
	            	// fix later jjchu, 
	            	
	            	ValueMetaInterface returned;
	            	if(data.db.getReturnRowMeta()!=null)
	            		returned = data.db.getReturnRowMeta().getValueMeta(i);
	            	else
	            		returned = null;	            	
	                ValueMetaInterface expected = data.returnMeta.getValueMeta(i);
...
//		}
	...
	public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException
	{
        ...
        if (first)
        {
            ...
            data.fuzzyMatchKeynrs= new int[meta.getTableFuzzyMatchKeyField().length];
            if(meta.getTableFuzzyMatchKeyField()!=null && meta.getTableFuzzyMatchKeyField().length>0)
            {
            	 for (int i=0;i<meta.getStreamFuzzyMatchKeyField().length;i++)
                 {
            		 data.fuzzyMatchKeynrs[i] = getInputRowMeta().indexOfValue(meta.getStreamFuzzyMatchKeyField()[i]);
                 }	
            }
		...
		try
		{
            ...
            //jason, fuzzy match when return multiple result rows; 
            //outputRow = lookupValues(getInputRowMeta(), r);
            if(meta.getTableFuzzyMatchKeyField() !=null && meta.getTableFuzzyMatchKeyField().length>0)
            	outputRow = lookupFuzzyMatchValues(getInputRowMeta(), r);
            else
            	outputRow = lookupValues(getInputRowMeta(), r);
            //end 
