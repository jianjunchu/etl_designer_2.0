...
public class DatabaseLookupMeta extends BaseStepMeta implements StepMetaInterface
{
	...
	public static final String[] matchStrings = new String[] { "Jaro", "Jaro Winglo" };  //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$ //$NON-NLS-7$ //$NON-NLS-8$ //$NON-NLS-9$ //$NON-NLS-10$ //$NON-NLS-11$
	...
	//add fuzzy match Jason
	/** the min score value for fuzzy match*/
	private String minValues[]; 
	
	/** fuzzy match field  in stream... */
	private String streamFuzzyMatchKeyField[];           

	/** fuzzy match field in table */
	private String tableFuzzyMatchKeyField[];            

	/** Fuzzy match method: Jaro ... */
	...
	public void allocate(int nrkeys, int nrvalues, int fuzzyMatch)
	{
		...
		tableFuzzyMatchKeyField        = new String[fuzzyMatch];
		streamFuzzyMatchKeyField        = new String[fuzzyMatch];
		minValues          = new String[fuzzyMatch];
		matchMethods = new String[fuzzyMatch];
	}
	...
	public Object clone()
	{
		...
		int nvMatchKeys = streamFuzzyMatchKeyField.length;
		
		retval.allocate(nrkeys, nrvalues,nvMatchKeys);
		...
		throws KettleXMLException
	{
		try
		{
			...
			int nrFuzzyMatchvalues = XMLHandler.countNodes(lookup, "fuzzy_match_key"); //$NON-NLS-1$
	
			allocate(nrkeys, nrvalues,nrFuzzyMatchvalues);
			...
			for (int i=0;i<nrFuzzyMatchvalues;i++)
			{
				Node knode = XMLHandler.getSubNodeByNr(lookup, "fuzzy_match_key", i); //$NON-NLS-1$
				
				tableFuzzyMatchKeyField         [i] = XMLHandler.getTagValue(knode, "table_field"); //$NON-NLS-1$
				matchMethods   [i] = XMLHandler.getTagValue(knode, "method"); //$NON-NLS-1$
				//if (matchMethods[i]==null) matchMethods[i]="Jaro"; //$NON-NLS-1$
				streamFuzzyMatchKeyField[i] = XMLHandler.getTagValue(knode, "stream_field"); //$NON-NLS-1$
				minValues        [i] = XMLHandler.getTagValue(knode, "min_value"); //$NON-NLS-1$
			}
	...
	public void setDefault()
	{
		...
		int nrvalues = 0;
		int nrFuzzyMatch = 0;
		allocate(nrkeys, nrvalues,nrFuzzyMatch);
	...
	public String getXML()
	{
		...
		for (int i=0;i<this.getTableFuzzyMatchKeyField().length;i++)
		{
			retval.append("      <fuzzy_match_key>").append(Const.CR); //$NON-NLS-1$
			retval.append("        ").append(XMLHandler.addTagValue("table_field", getTableFuzzyMatchKeyField()[i])); //$NON-NLS-1$ //$NON-NLS-2$
			retval.append("        ").append(XMLHandler.addTagValue("method", this.getMatchMethods()[i])); //$NON-NLS-1$ //$NON-NLS-2$
			retval.append("        ").append(XMLHandler.addTagValue("stream_field", getStreamFuzzyMatchKeyField()[i])); //$NON-NLS-1$ //$NON-NLS-2$
			retval.append("        ").append(XMLHandler.addTagValue("min_value", this.getMinValues()[i])); //$NON-NLS-1$ //$NON-NLS-2$
			retval.append("      </fuzzy_match_key>").append(Const.CR); //$NON-NLS-1$
		}
		...
		throws KettleException
	{
		try
		{
			...
			int nrFuzzyMatchKeys = rep.countNrStepAttributes(id_step, "match_stream_key"); //$NON-NLS-1$
			
			allocate(nrkeys, nrvalues,nrFuzzyMatchKeys);
			...
			{
				streamFuzzyMatchKeyField[i]       = rep.getStepAttributeString (id_step, i, "match_stream_key"); //$NON-NLS-1$
				tableFuzzyMatchKeyField[i]     = rep.getStepAttributeString (id_step, i, "match_table_key"); //$NON-NLS-1$
				matchMethods[i]     = rep.getStepAttributeString (id_step, i, "match_method"); //$NON-NLS-1$
				minValues[i] =  rep.getStepAttributeString(id_step, i, "match_values");  //$NON-NLS-1$
			}
		}
		catch(Exception e)
		{
		...
		throws KettleException
	{
		try
		{
			...
			for (int i=0;i<tableFuzzyMatchKeyField.length;i++)
			{
				rep.saveStepAttribute(id_transformation, id_step, i, "match_stream_key",   streamFuzzyMatchKeyField[i]); //$NON-NLS-1$
				rep.saveStepAttribute(id_transformation, id_step, i, "match_table_key",     tableFuzzyMatchKeyField[i]); //$NON-NLS-1$
				rep.saveStepAttribute(id_transformation, id_step, i, "match_method", matchMethods[i]); //$NON-NLS-1$
				rep.saveStepAttribute(id_transformation, id_step, i, "match_values",     minValues[i]); //$NON-NLS-1$
			}
	...
	public String[] getStreamFuzzyMatchKeyField() {
		return streamFuzzyMatchKeyField;
	}

	public void setStreamFuzzyMatchKeyField(String[] streamKeyField) {
		this.streamFuzzyMatchKeyField = streamKeyField;
	}
	...
	public String[] getTableFuzzyMatchKeyField() {
		return tableFuzzyMatchKeyField;
	}

	public void setTableFuzzyMatchKeyField(String[] tableFuzzyMatchKeyField) {
		this.tableFuzzyMatchKeyField = tableFuzzyMatchKeyField;
	}
