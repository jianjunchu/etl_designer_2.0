...
public class JobEntryPGPDecryptFiles extends JobEntryBase implements Cloneable, JobEntryInterface
{
	...
	public String getXML()
	{
		...
		if (source_filefolder!=null)
		{
			for (int i=0;i<source_filefolder.length;i++)
			{
				...
				retval.append("          ").append(XMLHandler.addTagValue("passphrase", Encr.encryptPasswordIfNotUsingVariables(passphrase[i])));
	...
	public Result execute(Result previousResult, int nr) throws KettleException 
	{
		try {
			...
			if(include_subfolders){
				if(isDetailed()) logDetailed(BaseMessages.getString(PKG, "JobPGPDecryptFiles.Log.IncludeSubFoldersOn"));
			}	
			...
			if(iffileexists.equals("move_file")){
				if(Const.isEmpty(MoveToFolder)){
				 ...
				 try{
					 ...
					 if(!folder.exists()) {
						 if(isDetailed()) logDetailed(BaseMessages.getString(PKG, "JobPGPDecryptFiles.Log.Error.FolderMissing",MoveToFolder));
					 ...
					 logError(BaseMessages.getString(PKG, "JobPGPDecryptFiles.Log.Error.GettingMoveToFolder",MoveToFolder,e.getMessage()));
					 return result; 
				 }finally{
					 if ( folder != null ){
							try {
			...
			if (arg_from_previous){
				if (isDetailed())
					logDetailed(BaseMessages.getString(PKG, "JobPGPDecryptFiles.Log.ArgFromPrevious.Found",(rows!=null?rows.size():0)+ ""));
			}
			if (arg_from_previous && rows!=null){
				for (int iteration=0;iteration<rows.size();iteration++) {			
					...
					String vwildcard_previous = Encr.decryptPasswordOptionallyEncrypted(environmentSubstitute(resultRow.getString(1, null)));
					...
					if(!Const.isEmpty(vsourcefilefolder_previous) &&  !Const.isEmpty(vdestinationfilefolder_previous)){
						if(isDetailed())
							logDetailed(BaseMessages.getString(PKG, "JobPGPDecryptFiles.Log.ProcessingRow",vsourcefilefolder_previous, vdestinationfilefolder_previous, vwildcard_previous));
	
						if(! ProcessFileFolder(vsourcefilefolder_previous,vpassphrase_previous, vdestinationfilefolder_previous,vwildcard_previous,parentJob,result,MoveToFolder))
						{
						...
						if(isDetailed())
							logDetailed(BaseMessages.getString(PKG, "JobPGPDecryptFiles.Log.IgnoringRow",vsourcefilefolder[iteration],vdestinationfilefolder[iteration],vwildcard[iteration]));
					}
				}
			}
				...
				for (int i=0;i<vsourcefilefolder.length && !parentJob.isStopped();i++){
					...
					if(!Const.isEmpty(vsourcefilefolder[i]) && !Const.isEmpty(vdestinationfilefolder[i])){
					  // ok we can process this file/folder
						if(isDetailed())
							logDetailed(BaseMessages.getString(PKG, "JobPGPDecryptFiles.Log.ProcessingRow",vsourcefilefolder[i],vdestinationfilefolder[i],vwildcard[i]));
					
						if(!ProcessFileFolder(vsourcefilefolder[i],Encr.decryptPasswordOptionallyEncrypted(environmentSubstitute(vpassphrase[i])), vdestinationfilefolder[i],vwildcard[i],parentJob,result,MoveToFolder)){
							// Update Errors
							updateErrors();
						}
					}
						...
						if(isDetailed())
							logDetailed(BaseMessages.getString(PKG, "JobPGPDecryptFiles.Log.IgnoringRow",vsourcefilefolder[i],vdestinationfilefolder[i],vwildcard[i]));
					}
				}
			}	
			...
			if(source_filefolder!=null) source_filefolder=null;
			if(destination_filefolder!=null) destination_filefolder=null;
		}
	}
	...
	private boolean ProcessFileFolder(String sourcefilefoldername, String passPhrase,
			String destinationfilefoldername,String wildcard,
			Job parentJob,Result result,String MoveToFolder)
	{
						...
						}else if (sourcefilefolder.getType().equals(FileType.FILE) && destination_is_a_file)
						{
							...
							if (fileObjects != null)  {
	                            for (int j = 0; j < fileObjects.length && !parentJob.isStopped(); j++)
	                            {
	                                ...
	                                if(!DecryptOneFile(Currentfile, sourcefilefolder,passPhrase, realDestinationFilefoldername, 
					...
					// Folders..only if include subfolders
					 if (Currentfile.getType() == FileType.FOLDER)
					 {
						/* if (include_subfolders && move_empty_folders && Const.isEmpty(wildcard))
						 {
							 entrystatus=DecryptFile(shortfilename,Currentfile,file_name,movetofolderfolder,log,parentJob,result);
						 }	*/		 
					 }
					 else
					 {
	...
	private boolean CreateDestinationFolder(FileObject filefolder)
	{
		...
		try
		{
    		...
    		if(!folder.exists())	
    		{
    			...
    			{
    				if(isDetailed()) logDetailed(BaseMessages.getString(PKG, "JobPGPDecryptFiles.Log.FolderNotExist",folder.getName().toString()));
    				folder.createFolder();
        			if(isDetailed()) logDetailed(BaseMessages.getString(PKG, "JobPGPDecryptFiles.Log.FolderWasCreated", folder.getName().toString()));
    			}
    			else
    			{
