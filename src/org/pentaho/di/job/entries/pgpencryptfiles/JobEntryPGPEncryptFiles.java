...
public class JobEntryPGPEncryptFiles extends JobEntryBase implements Cloneable, JobEntryInterface
{
	...
	public static final String[] actionTypeCodes= new String[] { 
		"encrypt", 
		"sign",
		"signandencrypt"
	};
	...
	public String getXML()
	{
		...
		if (source_filefolder!=null)
		{
			for (int i=0;i<source_filefolder.length;i++)
			{
				...
				retval.append("          ").append(XMLHandler.addTagValue("action_type",getActionTypeCode(action_type[i])));
	...
	public void loadRep(Repository rep, ObjectId id_jobentry, List<DatabaseMeta> databases, List<SlaveServer> slaveServers) throws KettleException
	{
		...
		catch(KettleException dbe)
		{
			...
			throw new KettleException(BaseMessages.getString(PKG, "JobPGPEncryptFiles.Error.Exception.UnableLoadRep")+id_jobentry, dbe);
		}
	}
	...
	public Result execute(Result previousResult, int nr) 
	{
		...
		try {
			...
			if(include_subfolders){
				if(isDetailed()) logDetailed(BaseMessages.getString(PKG, "JobPGPEncryptFiles.Log.IncludeSubFoldersOn"));
			}	
			...
			if(iffileexists.equals("move_file")){
				if(Const.isEmpty(MoveToFolder)){
				 ...
				 try{
					 ...
					 if(!folder.exists()) {
						 if(isDetailed()) logDetailed(BaseMessages.getString(PKG, "JobPGPEncryptFiles.Log.Error.FolderMissing",MoveToFolder));
	...
	public static int getActionTypeByDesc(String tt) {
	 ...
	 public static String getActionTypeDesc(int i) {
		 if(i<0 && i> ACTION_TYPE_SIGN_AND_ENCRYPT) return actionTypeDesc[0];
						...
						}else if (sourcefilefolder.getType().equals(FileType.FILE) && destination_is_a_file)
						{
							...
							if (fileObjects != null)  {
	                            for (int j = 0; j < fileObjects.length && !parentJob.isStopped(); j++)
	                            {
	                                ...
	                                if(!EncryptOneFile(actionType, Currentfile, sourcefilefolder,realuserID, realDestinationFilefoldername, 
			...
			Job parentJob,Result result)
	{
		...
		try{
			if(!destinationfilename.exists()){
				...
				if(isDetailed()) logDetailed(BaseMessages.getString(PKG, "JobPGPEncryptFiles.Log.FileEncrypted",sourcefilename.getName().toString(),destinationfilename.getName().toString()));
				...
				if(isDetailed()) logDetailed(BaseMessages.getString(PKG, "JobPGPEncryptFiles.Log.FileExists",destinationfilename.toString()));
				if(iffileexists.equals("overwrite_file")){
					doJob(actionType, sourcefilename, userID, destinationfilename);
					if(isDetailed()) logDetailed(BaseMessages.getString(PKG, "JobPGPEncryptFiles.Log.FileOverwrite",destinationfilename.getName().toString()));
					...
					{
						logError(BaseMessages.getString(PKG, "JobPGPEncryptFiles.Error.GettingFilename",short_filename));
						return retval;
					}
					...
					doJob(actionType, sourcefilename, userID, destinationfilename);
					if(isDetailed()) logDetailed(toString(), BaseMessages.getString(PKG, "JobPGPEncryptFiles.Log.FileEncrypted",sourcefilename.getName().toString(),destinationfile.getName().toString()));
					...
					if(isDetailed()) logDetailed(BaseMessages.getString(PKG, "JobPGPEncryptFiles.Log.FileDeleted",destinationfilename.getName().toString()));
				}
					...
					if(!destinationfile.exists()){
						...
						if(isDetailed()) logDetailed(BaseMessages.getString(PKG, "JobPGPEncryptFiles.Log.FileEncrypted",sourcefilename.getName().toString(),destinationfile.getName().toString()));
						...
						if(ifmovedfileexists.equals("overwrite_file")) {
							sourcefilename.moveTo(destinationfile);
							if(isDetailed()) logDetailed(BaseMessages.getString(PKG, "JobPGPEncryptFiles.Log.FileOverwrite",destinationfile.getName().toString()));
							...
							sourcefilename.moveTo(destinationfile);
							if(isDetailed()) logDetailed(BaseMessages.getString(PKG, "JobPGPEncryptFiles.Log.FileEncrypted",destinationfile.getName().toString()));
	...
	try {
     	if (!Currentfile.toString().equals(sourcefilefolder.toString()))
		{
			...
			{
				logError(BaseMessages.getString(PKG, "JobPGPEncryptFiles.Error.GettingFilename",Currentfile.getName().getBaseName(),e.toString()));
				return entrystatus;
			}
				 ...
				 if (include_subfolders)
				 {
					// Folders..only if include subfolders
					 ...
					 else
					 {
						...
						{	
							entrystatus=EncryptFile(actionType, shortfilename,Currentfile,userID, file_name,movetofolderfolder,parentJob,result);
						}
					 }
				 }
			 }
 ...
 return entrystatus;
	...
	private boolean CreateDestinationFolder(FileObject filefolder)
	{
		...
		try
		{
			...
			logError(BaseMessages.getString(PKG, "JobPGPEncryptFiles.Log.CanNotCreateParentFolder",folder.getName().toString()));
