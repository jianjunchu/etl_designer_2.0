...
//		KettleEnvironment.init();
        ...
        StringBuffer optionLogfile, optionLogfileOld, optionListdir, optionListjobs, optionListrep, optionNorep, optionVersion, optionListParam, optionExport,optionRandomDelay;;
    ...
    CommandLineOption maxLogTimeoutOption = new CommandLineOption("maxlogtimeout", BaseMessages.getString(PKG, "Kitchen.CmdLine.MaxLogTimeout"), new StringBuffer()); //$NON-NLS-1$ //$NON-NLS-2$
		...
		CommandLineOption options[] = new CommandLineOption[] 
            {
		        ...
		        new CommandLineOption("randomdelay", BaseMessages.getString(PKG, "Kitchen.ComdLine.RandomDelay"), optionRandomDelay=new StringBuffer()),
		        maxLogLinesOption,
		        maxLogTimeoutOption,
            };
        ...
        //thread sleep for a random time.
        if (!Const.isEmpty(optionRandomDelay)) {
			int delay = Integer.parseInt(optionRandomDelay.toString());
			Random random = new Random();
			int  delayTime = new Float(random.nextFloat() *delay*60*1000).intValue();
			try {
			...
			if (!Const.isEmpty(optionRepname) || !Const.isEmpty(optionFilename))
			{
				...
				if (!Const.isEmpty(optionRepname) && !"Y".equalsIgnoreCase(optionNorep.toString()))
				{
					...
					try {
					    ...
					    repUrl = repUrl + "/login?action=login&user_name=" + optionUsername.toString() + "&password=" + optionPassword.toString();
					    repsinfo.testConnection(repUrl);
					    //xnren end
						repsinfo.readData();
					} catch(Exception e) {
