...
import org.pentaho.di.repository.kdr.delegates.KettleDatabaseRepositoryCacheFileDelegate;
...
//	private static Class<?> PKG = Repository.class; // for i18n purposes, needed by Translator2!!   $NON-NLS-1$
	...
	public KettleDatabaseRepositoryCacheFileDelegate cacheFileDelegate;
  ...
  public void init(RepositoryMeta repositoryMeta) {
	...
	private void init() {
		...
		this.cacheFileDelegate = new KettleDatabaseRepositoryCacheFileDelegate(this);
	}
  ...
  private void registerRepositoryService(Class<? extends IRepositoryService> clazz, IRepositoryService repositoryService) {
    ...
    public void save(RepositoryElementInterface repositoryElement, String versionComment, ProgressMonitorListener monitor, ObjectId parentId, boolean used, boolean overwrite) throws KettleException {
    	...
    	try {
    		...
    		case PARTITION_SCHEMA :
	    		...
	    		/**
	    		 * add by cli
	    		 * save cache file to database
	    		 */
    		case CACHE_FILE:
    			cacheFileDelegate.saveCacheFile((CacheFile)repositoryElement, parentId, used, overwrite);
    			break;
    			//cli edit end
	    	default:
    ...
    public CacheFile loadCacheFile(ObjectId id_cache_file, String versionName) throws KettleException {
    	return cacheFileDelegate.loadCacheFile(id_cache_file);
    }
  ...
  public RepositoryDirectoryInterface findDirectory(ObjectId directory) throws KettleException {
    ...
    //add by cli
    public ObjectId[] getCacheFileIDs(ObjectId id_cache_file) throws KettleException
    {
        return connectionDelegate.getIDs("SELECT "+quote(KettleDatabaseRepository.FIELD_ID_CACHE_FILE)+" FROM "+quoteTable(KettleDatabaseRepository.TABLE_R_CACHE_FILE)+" WHERE "+quote(KettleDatabaseRepository.FIELD_ID_CACHE_FILE)+" = ? ", id_cache_file);
    }
    ...
    //edit by cli
    public ObjectId[] getCacheFileIDs(boolean includeDeleted) throws KettleException
    {
        return connectionDelegate.getIDs("SELECT "+quote(KettleDatabaseRepository.FIELD_ID_CACHE_FILE)+" FROM "+quoteTable(KettleDatabaseRepository.TABLE_R_CACHE_FILE));
    }
  ...
  public static final String byteArrayToString(byte[] val) throws IOException {
    ...
    //add by cli
    public synchronized void deleteCacheFile(ObjectId id_cache_file) throws KettleException {
    	cacheFileDelegate.delCacheFile(id_cache_file);
    	commit();
    }
